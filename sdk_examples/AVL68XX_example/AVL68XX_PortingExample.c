...
static AVL_Tuner global_tuner_t_c = 
{
    ...
    MxL608_Initialize,                              //tuner initialization function
    MxL608_Lock,                                   //tuner lock function
    MxL608_GetLockStatus,                     //tuner check lock status function
    MxL608_GetRFStrength,
...
static AVL_Tuner global_tuner_s = 
{
    ...
    ExtAV2011_Initialize,                              //tuner initialization function
    ExtAV2011_Lock,                                   //tuner lock function
    ExtAV2011_GetLockStatus,                     //tuner check lock status function
    NULL,
};
	...
	*pLockFlag = 0;
    ...
    while(times_out_cnt<200)//this time-out window can be customized
    {
		...
		else if(eCurrentDemodMode ==  AVL_ISDBT)
		{
		   ...
		   if(ucNosignal == 0)//ucNosignal=1:signal exist 0:nosignal
           {
             printf("[AVL_Check_LockStatus] ----ISDBT NoSignal Channel! ---- \n");
             break;
           }
		}
#if 0
		else if(eCurrentDemodMode ==  AVL_DVBC)
		{
		   ...
		   if(ucNosignal == 0)//ucNosignal=1:signal exist 0:nosignal
           {
             printf("[AVL_Check_LockStatus] ----DVBC NoSignal Channel! ---- \n");
             break;
           }
		}
#endif
...
static AVL_ErrorCode AVL_Init(void)
{
    ...
    AVL_uint32 uiFamilyID = 0;

    return_code = AVL_Demod_GetChipID(&uiChipID,uiChipNo);
    if(return_code == AVL_EC_OK)
	{
		if(uiChipID == 0xb) uiChipID = 6882;
		else if(uiChipID == 0xf) uiChipID = 6862;
		else if(uiChipID == 0xd) uiChipID = 6812;
		else if(uiChipID == 0xe) uiChipID = 6762;
        printf("[GetChipId] chip id:AVL%d\n",uiChipID);
	}
    else
    ...
    if(return_code == AVL_EC_OK)
        printf("[GetFamilyId] Family ID:0x%x\n",(uiFamilyID<<16)>>16);
    else
        printf("[GetFamilyId] get Family ID Fail.\n");
...
static void  ISDBT_EWBSCheck_Example(void)
{
    AVL_uint32 Test_Times = 0;
    AVL_GPIOPinValue  ePinValue = AVL_LOGIC_0;
    ...
    {  
      ...
      AVL_Demod_GetGPIOValue( AVL_Pin15, &ePinValue, 0);

      if(ePinValue == (AVL_GPIOPinValue)1)
      {
         printf("Alert broadcasting!!\n");
         ISDBT_Reset_EWBSChangeFlag_Demod(0);
         // AVL_Demod_SetGPIO(AVL_Pin15, AVL_LOGIC_0, 0);
      }
       else if (ePinValue == (AVL_GPIOPinValue)0)
       {
          printf("NO ALert broadcasting!!\n");
       }
        printf("\n");
    }
}
...
//======================================================//
    ...
    AVL_LockChannel_DVBC(666*1000,6875);
    AVL_Check_LockStatus(&nLockFlag);
//======================================================//
